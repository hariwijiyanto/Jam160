...
// Fungsi perkalian 256-bit penuh
__device__ __forceinline__ void bigint_mul(BigInt *res, const BigInt *a, const BigInt *b) {
    uint64_t product[2 * BIGINT_WORDS] = {0};
    
    // Schoolbook multiplication
    for (int i = 0; i < BIGINT_WORDS; i++) {
        uint64_t carry = 0;
        for (int j = 0; j < BIGINT_WORDS; j++) {
            uint64_t tmp = (uint64_t)a->data[i] * b->data[j] + product[i + j] + carry;
            product[i + j] = tmp & 0xFFFFFFFF;
            carry = tmp >> 32;
        }
        product[i + BIGINT_WORDS] = carry;
    }
    
    // Copy hasil 256-bit LSB
    for (int i = 0; i < BIGINT_WORDS; i++) {
        res->data[i] = (uint32_t)product[i];
    }
}

// Fungsi perkalian BigInt dengan uint64_t
__device__ __forceinline__ void bigint_mul_u64(BigInt *res, const BigInt *a, uint64_t b) {
    BigInt b_bigint;
    init_bigint_from_u64(&b_bigint, b);
    bigint_mul(res, a, &b_bigint);
}
